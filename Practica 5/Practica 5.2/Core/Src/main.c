...
int main(void)
{
  ...
  if (!uartInit())
  {
      // Manejo de error si la inicialización de la UART falla
      while (1)
      {
  ...
  while (1)
  {
      ...
      if (readKey())
      {
          ...
          if (toggleFrequency == 100)
          {
              ...
              // Enviar mensaje por la UART cuando se detecta un flanco ascendente
          ...
          {
              toggleFrequency = 100;
              // Enviar mensaje por la UART cuando se detecta un flanco descendente
          ...
          // Esperar un tiempo para que los LEDs estén encendidos
          HAL_Delay(100);
