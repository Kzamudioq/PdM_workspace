...
int main(void)
{
  ...
  delayInit(&ledDelay, 200); // Inicializar el retardo no bloqueante para los LEDs con una duración de 200 ms
  /* USER CODE END 2 */
...
void Toggle_LED_Sequence(uint8_t sequenceIndex)
{
  ...
  for (int i = 0; i < 3; i++) {
    ...
    delayWrite(&ledDelay, 200); // Iniciar el retardo no bloqueante para mantener el LED encendido durante 200 ms
    ...
    delayWrite(&ledDelay, 200); // Iniciar el retardo no bloqueante para mantener el LED apagado durante 200 ms
    while (!delayRead(&ledDelay)) {
      // Esperar hasta que el retardo no bloqueante haya finalizado
    }
  }
